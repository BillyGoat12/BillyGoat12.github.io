...
_.partition = function (array, test) {
    ...
    arr.push(_.filter(array, test));// push the output of filter function with arguements array and test into arr array
    arr.push(_.reject(array, test));// push the output of reject function with arguements array and test into arr array
...
_.map = function(collection, test) {
    let arr = [];// created a new array
    _.each(collection, (val, index, collection) => {// loop over the array
...
_.pluck = function(array, key) {
    return _.map(array, val => {// loop over the array and return new array
...
_.every = function (collection, test) {
    ...
    if(!test){// check if there is no test
        _.each(collection, val => {// loop over the collection
    ...
    _.each(collection, (val, property, collection) => {// loop over the collection
...
_.reduce = function(array, test, seed) {
    if(seed === undefined){// checking if seed equal undefined
        _.each(array, (val, i, collection) => {// looping over the array
        ...
        _.each(array, (val, i, collection) => {// loop over the array
            seed = test(seed, val, i);// setting seed equal to the output of test with the arguement seed value and index
        });
    }
